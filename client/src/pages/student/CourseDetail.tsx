...
export default function StudentCourseDetail() {
          ...
          const modulesWithAssignments = await Promise.all(
            course.modules.map(async (module: any) => {
              ...
              try {
                    ...
                    assignments.map(async (assignment: any) => {
                      try {
                        const submissionResponse = await fetch(
                          `/api/submissions?assignmentId=${assignment.id}&student=me`,
                          {
                            credentials: "include",
                          },
                        );
                ...
                console.error("Error fetching assignments for module:", error);
                return {
                  ...module,
                  assignments: [],
                };
  ...
  // Calculate course progress with null safety
  useEffect(() => {
    if (analytics && course) {
      // Add null safety for courseProgress
      const progress = analytics?.courseProgress?.[course.id] || 0;
      setCourseProgress(progress);
    }
  }, [analytics, course]);

  // Handle case when we have course but no analytics yet
  useEffect(() => {
    if (course && !analytics && !loadingAnalytics) {
      setCourseProgress(0); // Default to 0 when no analytics available
    }
  }, [course, analytics, loadingAnalytics]);
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="w-full lg:w-1/3 space-y-6">
            <Card>
              ...
              <CardContent>
                <p className="text-neutral-700 mb-4 dark:text-neutral-300">
                  {course.description}
                </p>

                <div className="border-t border-neutral-200 pt-4 mt-4 dark:border-neutral-700">
                  <h4 className="font-medium text-neutral-800 mb-2 dark:text-neutral-200">
                    Instructor
                  </h4>

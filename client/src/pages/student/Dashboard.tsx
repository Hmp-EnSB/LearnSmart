...
export default function StudentDashboard() {
  ...
  useEffect(() => {
    if (enrollments && analytics) {
      ...
      if (courses.length > 0) {
        const totalProgress = courses.reduce((sum: number, course: EnrolledCourse) => sum + course.progress, 0);
  ...
  useEffect(() => {
    if (enrollments) {
      ...
      Promise.all(
        ...
        courseAssignmentsArray.forEach((courseAssignments: any[], index: number) => {
  ...
  return (
    <MainLayout title="Student Dashboard">
      <div className="space-y-6">
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <Card>
            <CardContent className="p-6">
              <h2 className="font-heading font-semibold text-neutral-800 dark:text-neutral-200">Enrolled Courses</h2>
              <p className="text-3xl font-bold text-primary mt-2">{isLoading ? '-' : enrolledCourses.length}</p>
            </CardContent>
          </Card>
          
          <Card>
            <CardContent className="p-6">
              <h2 className="font-heading font-semibold text-neutral-800 dark:text-neutral-200">Pending Assignments</h2>
              <p className="text-3xl font-bold text-secondary-500 mt-2">{isLoading ? '-' : pendingAssignments.length}</p>
            </CardContent>
          </Card>
          
          <Card>
            <CardContent className="p-6">
              <h2 className="font-heading font-semibold text-neutral-800 dark:text-neutral-200">Badges Earned</h2>
              <p className="text-3xl font-bold text-success-500 mt-2">{isLoading ? '-' : badges?.length || 0}</p>
            </CardContent>
          </Card>
          
          <Card>
            <CardContent className="p-6">
              <h2 className="font-heading font-semibold text-neutral-800 dark:text-neutral-200">Overall Progress</h2>
              <div className="mt-2">
                <ProgressBar value={overallProgress} size="md" />
                <p className="text-sm text-neutral-600 dark:text-neutral-400 mt-1">{overallProgress}% complete</p>
              </div>
            </CardContent>
          </Card>
        </section>
              ...
              enrolledCourses.slice(0, 3).map(course => (
                <Card key={course.id} className="overflow-hidden">
                  <div className="h-40 bg-neutral-200 dark:bg-neutral-700">
                    {/* This is a placeholder image - in a real app, you'd use course.imageUrl */}
                  ...
                  <CardContent className="p-6">
                    ...
                    <div className="mt-4 flex justify-between items-center">
                      <div className="w-3/4">
                        ...
                        <p className="text-xs text-neutral-500 dark:text-neutral-500 mt-1">{course.progress}% complete</p>
                      </div>
                      <Link href={`/student/courses/${course.slug}`}>
                        <a className="text-primary hover:text-primary/90 font-medium text-sm">Continue</a>
                      </Link>
                    </div>
                  </CardContent>
                </Card>
        ...
        <section>
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-xl font-heading font-semibold text-neutral-800 dark:text-neutral-200">Upcoming Assignments</h2>
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-neutral-200 dark:divide-neutral-700">
                <thead className="bg-neutral-50 dark:bg-neutral-800">
                  <tr>
                    <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-neutral-500 uppercase tracking-wider dark:text-neutral-400">Assignment</th>
                    <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-neutral-500 uppercase tracking-wider dark:text-neutral-400">Course</th>
                    <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-neutral-500 uppercase tracking-wider dark:text-neutral-400">Due Date</th>
                    <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-neutral-500 uppercase tracking-wider dark:text-neutral-400">Status</th>
                    <th scope="col" className="px-6 py-3 text-right text-xs font-medium text-neutral-500 uppercase tracking-wider dark:text-neutral-400">Action</th>
                  </tr>
                </thead>
                    ...
                    pendingAssignments.map(assignment => (
                      <tr key={assignment.id}>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="text-sm font-medium text-neutral-900 dark:text-neutral-200">{assignment.title}</div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="text-sm text-neutral-500 dark:text-neutral-400">{assignment.course.title}</div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                          <Link href={`/student/assignments/${assignment.id}`}>
                            <a className="text-primary hover:text-primary/90">Start</a>
                          </Link>
                        </td>
                      </tr>
                    ))
        ...
        <section>
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-xl font-heading font-semibold text-neutral-800 dark:text-neutral-200">Achievement Badges</h2>
          </div>
              ...
              badges.slice(0, 6).map((item: BadgeItem) => (
                <Card key={item.badge.id} className="p-4 text-center">
                  <div className="w-12 h-12 mx-auto bg-primary-100 text-primary rounded-full flex items-center justify-center dark:bg-primary-900">
                    ...
                    <span className="text-lg font-bold">{item.badge.name.substring(0, 1)}</span>
                  </div>

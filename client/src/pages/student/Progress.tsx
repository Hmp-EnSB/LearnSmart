...
export default function StudentProgress() {
  ...
  useEffect(() => {
    if (enrollments && analytics) {
      ...
      courses.sort(
        (a: CourseProgress, b: CourseProgress) => b.progress - a.progress,
      );
      ...
      if (courses.length > 0) {
        const total = courses.reduce(
          (sum: number, course: CourseProgress) => sum + course.progress,
          0,
        );
                ...
                <div className="flex gap-3">
                  ...
                  <div>
                    <div className="text-3xl font-bold text-green-600">
                      {
                        courseProgress.filter((c) => c.status === "completed")
                          .length
                      }
                    </div>
                    <div className="text-sm text-neutral-500 dark:text-neutral-400">
          ...
          <TabsContent value="achievements" className="space-y-6">
            ...
            {isLoading ? (
              ...
              <Card>
                <CardContent className="py-6 text-center">
                  <p className="text-neutral-600 dark:text-neutral-400">
                    You haven't earned any badges yet. Complete courses and
                    assignments to earn them!
                  </p>
                </CardContent>
              </Card>
            ) : (
                ...
                {badges.map((item: BadgeItem) => (
                  <Card key={item.badge.id} className="p-4 text-center">
                    ...
                    <p className="text-xs text-neutral-400 mt-2 dark:text-neutral-500">
                      Earned on{" "}
                      {new Date(item.userBadge.awardedAt).toLocaleDateString()}
                    </p>
                  </Card>
                ))}
              </div>
                    ...
                    <div className="text-center p-4 bg-neutral-50 rounded-lg dark:bg-neutral-800">
                      <div className="text-3xl font-bold text-blue-500">
                        {submissionStats.total > 0
                          ? Math.round(
                              (submissionStats.onTime / submissionStats.total) *
                                100,
                            )
                          : 0}
                        %
                  ...
                  <div className="flex items-center space-x-4">
                    ...
                    <div className="flex-1">
                      <div className="flex justify-between">
                        ...
                        <div className="text-sm font-medium text-neutral-800 dark:text-neutral-200">
                          {
                            courseProgress.filter((c) => c.status === "active")
                              .length
                          }
                        </div>
                      </div>
